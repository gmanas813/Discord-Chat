...
import { getStreamToken } from "../src/graphql/queries";
const AuthContext = createContext({
    userId:null,
    setId : (newUser) => {},
});

const AuthContextComp = ({children,client}) => {
    const [userId,setId] = useState(null);

    connectChatUser = async () => {
        const userData = await Auth.currentAuthenticatedUser();
        const {sub,name}=userData.attributes;
        const token=await API.graphql(graphqlOperation(getStreamToken));
